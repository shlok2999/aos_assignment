...
void * download_file(void *arg)
{
    ...
    for(string peer:peers)
    {
        ...
        //while(1)
        //{
            int n=recv(newconnect,buffer+file_pointer,1024,0);
            // if(n<=0)
            //     break; 
            // file_pointer+=n;
            // cout<<n<<endl;
    ...
    vector<pair<int,vector<string>>> chunks(num_of_chunks);
    for(int i=0;i<details.size();i++)
    {
        for(int j=0;j<details[i].size();j++)
        {
            if(details[i][j]=='1')
            {
                chunks[j].second.push_back(peers[i]);
                chunks[j].first=j;
            }
        }
    }

    sort(chunks.begin(),chunks.end(),comparator);
    ...
    int fd=open(file_desc,O_WRONLY | O_CREAT, S_IRUSR| S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH);
        int n=chunks[i].second.size();
        int index=rand()%n;
        chunk_detail c(chunks[i].first,chunks[i].second[index],header[0],fd,"");
        //pthread_create(&threads[i-1],NULL,download_chunk,c);
        download_chunk(c);
        close(fd);
        files_shared[header[0]]->bitmap[chunks[i].first]='1';
        //usleep(100);
    }
